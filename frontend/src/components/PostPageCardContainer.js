...
class PostPageCardContainer extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (this._isMounted && prevProps.location !== this.props.location) {
      this.getPosts();
    }
  }
      ...
      <Col md={8}>
        ...
        <nav aria-label="Page navigation example">
          <ul className="pagination">
            <li className={this.getCurPage() <= 1 ? "page-item disabled" : "page-item" }>
              <Link to={this.getPrevPageUrl()} className="page-link" > Previous </Link>
            </li>
            <li className={this.getCurPage() >= this.state.pageCount? "page-item disabled": "page-item" }>
              <Link to={this.getNextPageUrl()} className="page-link" > Next </Link>
            </li>
          </ul>
        </nav>
      </Col>
